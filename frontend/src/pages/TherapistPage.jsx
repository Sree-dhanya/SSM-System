  ...
  const uploadPhoto = async () => {
    ...
    try {
      ...
      const resp = await axios.post(
        `${API_BASE_URL}/api/v1/therapists/${id}/photo`,
        formData,
        {
          headers,
  ...
  const handleEditToggle = () => {
    if (!isEditing) {
      ...
      setEditFormData({
        ...
        rci_renewal_date:
          therapist.rci_renewal_date_raw || therapist.rciRenewalDate,
        blood_group: therapist.bloodGroup,
        category: therapist.category,
        aadhar_number: therapist.aadhar_raw
          ? formatAadhaar(therapist.aadhar_raw)
          : therapist.aadhar,
